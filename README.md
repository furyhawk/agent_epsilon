...
└── commands/             # Click CLI commands (auto-discovered by `agent_epsilon cmd …`)
...
The generated project ships a Click CLI exposed as `agent_epsilon` (after `make install`):

```bash
agent_epsilon server run --reload          # dev server
agent_epsilon db upgrade                   # apply migrations
agent_epsilon db migrate -m "message"      # create new migration
agent_epsilon user create-admin            # interactive admin creation
agent_epsilon rag-ingest <path> -c docs    # ingest local files
agent_epsilon rag-search "query" -c docs   # semantic search
agent_epsilon rag-collections              # list collections
```

Run `make help` for a categorized list, or `agent_epsilon --help` for full CLI docs.
...
agent_epsilon rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
agent_epsilon rag-search "your query" --collection documents
```
